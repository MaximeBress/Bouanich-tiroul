...
if (!function_exists('jardiwinery_essgrids_theme_setup')) {
	...
	function jardiwinery_essgrids_theme_setup() {
		...
		if (jardiwinery_exists_essgrids()) {
			if (is_admin()) {
				add_filter( 'jardiwinery_filter_importer_options',			'jardiwinery_essgrids_importer_set_options', 10, 1 );
				add_action( 'jardiwinery_action_importer_params',			'jardiwinery_essgrids_importer_show_params', 10, 1 );
				add_action( 'jardiwinery_action_importer_import',			'jardiwinery_essgrids_importer_import', 10, 2 );
				add_action( 'jardiwinery_action_importer_import_fields',	'jardiwinery_essgrids_importer_import_fields', 10, 1 );
			}
		}
		if (is_admin()) {
...
if ( !function_exists( 'jardiwinery_essgrids_importer_set_options' ) ) {
	//add_filter( 'jardiwinery_filter_importer_options',	'jardiwinery_essgrids_importer_set_options', 10, 1 );
	function jardiwinery_essgrids_importer_set_options($options=array()) {
		if ( in_array('essgrids', jardiwinery_storage_get('required_plugins')) && jardiwinery_exists_essgrids() ) {
...
if ( !function_exists( 'jardiwinery_essgrids_importer_show_params' ) ) {
	//add_action( 'jardiwinery_action_importer_params',	'jardiwinery_essgrids_importer_show_params', 10, 1 );
	function jardiwinery_essgrids_importer_show_params($importer) {
		?>
		<input type="checkbox" <?php echo in_array('essgrids', jardiwinery_storage_get('required_plugins')) && $importer->options['plugins_initial_state'] 
											? 'checked="checked"' 
											: ''; ?> value="1" name="import_essgrids" id="import_essgrids" /> <label for="import_essgrids"><?php esc_html_e('Import Ess.Grids', 'jardiwinery'); ?></label><br>
		<?php
	}
}

// Import posts
if ( !function_exists( 'jardiwinery_essgrids_importer_import' ) ) {
	//add_action( 'jardiwinery_action_importer_import',	'jardiwinery_essgrids_importer_import', 10, 2 );
	function jardiwinery_essgrids_importer_import($importer, $action) {
		if ( $action == 'import_essgrids' ) {
			$attempt = !empty($_POST['attempt']) ? (int) $_POST['attempt']+1 : 1;
			if ($importer->options['debug']) 
				dfl( sprintf(esc_html__('Import Essential Grid "%s". Attempt %d.', 'jardiwinery'), $file, $attempt) );
			$data = jardiwinery_get_local_or_remote_file($importer->options['files'][$importer->options['demo_type']]['file_with_essgrid']);
			if (empty($data)) {
				$msg = sprintf(esc_html__('Essential Grid "%s" import error. Attempt %d.', 'jardiwinery'), $importer->options['files'][$importer->options['demo_type']]['file_with_essgrid'], $attempt);
				if ($attempt < 3) {
				...
				try {
					...
					if ($importer->options['debug']) 
						dfl( esc_html__('Essential Grid import complete', 'jardiwinery') );
							
				} catch (Exception $d) {
					$msg = sprintf(esc_html__('Essential Grid import error: %s', 'jardiwinery'), $d->getMessage());
					$importer->response['error'] = $msg;
					if ($importer->options['debug']) 
						dfl( $msg );
...
if ( !function_exists( 'jardiwinery_essgrids_importer_import_fields' ) ) {
	//add_action( 'jardiwinery_action_importer_import_fields',	'jardiwinery_essgrids_importer_import_fields', 10, 1 );
	function jardiwinery_essgrids_importer_import_fields($importer) {
		?>
		<tr class="import_essgrids">
			<td class="import_progress_item"><?php esc_html_e('Essential Grid', 'jardiwinery'); ?></td>
