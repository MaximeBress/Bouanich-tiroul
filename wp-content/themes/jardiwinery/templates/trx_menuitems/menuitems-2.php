...
if ( !function_exists( 'jardiwinery_template_menuitems_2_theme_setup' ) ) {
	add_action( 'jardiwinery_action_before_init_theme', 'jardiwinery_template_menuitems_2_theme_setup', 1 );
	function jardiwinery_template_menuitems_2_theme_setup() {
		jardiwinery_add_template(array(
			'layout' => 'menuitems-2',
			'template' => 'menuitems-2',
			'mode'   => 'menuitems',
			'title'  => esc_html__('MenuItems /Style 2/', 'jardiwinery'),
...
// Template output
if ( !function_exists( 'jardiwinery_template_menuitems_2_output' ) ) {
	function jardiwinery_template_menuitems_2_output($post_options, $post_data) {
			...
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				if (   ( $post_options['menuitem_calories'] != 'inherit'		&& strlen($post_options['menuitem_calories']) != 0 )
					...
					<div class="sc_menuitem_nutritions">
						...
						<ul class="sc_menuitem_nutritions_list">
							<?php
							if ( $post_options['menuitem_calories'] != 'inherit' && strlen($post_options['menuitem_calories']) != 0 )
								echo '<li>' . esc_html__('Calories: ', 'jardiwinery') . '<span>' . trim($post_options['menuitem_calories']) . esc_html__('Kcal', 'jardiwinery') . '</span></li>';
							if ( $post_options['menuitem_cholesterol'] != 'inherit' && strlen($post_options['menuitem_cholesterol']) != 0 ) {
								echo '<li>' . esc_html__('Cholesterol: ', 'jardiwinery') . '<span>' . trim($post_options['menuitem_cholesterol']) . esc_html__('mg', 'jardiwinery') . '</span></li>';
							}
							if ( $post_options['menuitem_fiber'] != 'inherit' && strlen($post_options['menuitem_fiber']) != 0 ) {
								echo '<li>' . esc_html__('Fiber: ', 'jardiwinery') . '<span>' . trim($post_options['menuitem_fiber']) . esc_html__('g', 'jardiwinery') . '</span></li>';
							}
							if ( $post_options['menuitem_sodium'] != 'inherit' && strlen($post_options['menuitem_sodium']) != 0 ) {
								echo '<li>' . esc_html__('Sodium: ', 'jardiwinery') . '<span>' . trim($post_options['menuitem_sodium']) . esc_html__('mg', 'jardiwinery') . '</span></li>';
							}
							if ( $post_options['menuitem_carbohydrates'] != 'inherit' && strlen($post_options['menuitem_carbohydrates']) != 0 ) {
								echo '<li>' . esc_html__('Carbohydrates: ', 'jardiwinery') . '<span>' . trim($post_options['menuitem_carbohydrates']) . esc_html__('g', 'jardiwinery') . '</span></li>';
							} 
							if ( $post_options['menuitem_fat'] != 'inherit' && strlen($post_options['menuitem_fat']) != 0 ) { 
								echo '<li>' . esc_html__('Fat: ', 'jardiwinery') . '<span>' . trim($post_options['menuitem_fat']) . esc_html__('g', 'jardiwinery') . '</span></li>';
							}
							if ( $post_options['menuitem_protein'] != 'inherit' && strlen($post_options['menuitem_protein']) != 0 ) {
								echo '<li>' . esc_html__('Protein: ', 'jardiwinery') . '<span>' . trim($post_options['menuitem_protein']) . esc_html__('g', 'jardiwinery') . '</span></li>';
							}
							?>
						</ul>
						<div class="cL"></div>
				...
				<div class="sc_menuitem_more">
					<?php if ( $post_options['menuitem_product'] != 'inherit' ) { ?>
						<a class="sc_button sc_button_square sc_button_style_filled sc_button_size_small margin_right_tiny" href="<?php echo get_permalink($post_options['menuitem_product']); ?>"><?php esc_html_e('ORDER', 'jardiwinery'); ?></a>
					<?php } ?>
					<a class="sc_button sc_button_square sc_button_style_filled sc_button_size_small" href="<?php echo esc_url($post_data['post_comments_link']); ?>"><?php esc_html_e('POST COMMENT', 'jardiwinery'); ?></a>
					<div class="cL"></div>
				</div>
