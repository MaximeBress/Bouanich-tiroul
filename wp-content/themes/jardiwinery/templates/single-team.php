...
if ( !function_exists( 'jardiwinery_template_single_team_theme_setup' ) ) {
	add_action( 'jardiwinery_action_before_init_theme', 'jardiwinery_template_single_team_theme_setup', 1 );
	function jardiwinery_template_single_team_theme_setup() {
		jardiwinery_add_template(array(
			'layout' => 'single-team',
			'mode'   => 'team',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single Team member', 'jardiwinery'),
...
// Template output
if ( !function_exists( 'jardiwinery_template_single_team_output' ) ) {
	function jardiwinery_template_single_team_output($post_options, $post_data) {
		$post_data['post_views']++;
		$show_title = jardiwinery_get_custom_option('show_post_title')=='yes';
		...
		jardiwinery_open_wrapper('<section class="post_content'.(!$post_data['post_protected'] && $post_data['post_edit_enable'] ? ' '.esc_attr('post_content_editor_present') : '').'" itemprop="articleBody">');
		...
		jardiwinery_close_wrapper();	// .post_item
