...
if ( !function_exists( 'jardiwinery_template_list_theme_setup' ) ) {
	add_action( 'jardiwinery_action_before_init_theme', 'jardiwinery_template_list_theme_setup', 1 );
	function jardiwinery_template_list_theme_setup() {
		jardiwinery_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'jardiwinery')
			));
	}
}

// Template output
if ( !function_exists( 'jardiwinery_template_list_output' ) ) {
	function jardiwinery_template_list_output($post_options, $post_data) {
