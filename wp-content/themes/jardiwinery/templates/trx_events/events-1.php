...
if ( !function_exists( 'jardiwinery_template_events_1_theme_setup' ) ) {
	add_action( 'jardiwinery_action_before_init_theme', 'jardiwinery_template_events_1_theme_setup', 1 );
	function jardiwinery_template_events_1_theme_setup() {
		jardiwinery_add_template(array(
			'layout' => 'events-1',
			'template' => 'events-1',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 1/', 'jardiwinery'),
...
// Template output
if ( !function_exists( 'jardiwinery_template_events_1_output' ) ) {
	function jardiwinery_template_events_1_output($post_options, $post_data) {
		$show_title = true;
							...
							if ($post_data['post_excerpt']) {
								echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
										...
										: '<p>'.trim(jardiwinery_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																										? $post_options['descr'] 
																										: jardiwinery_get_custom_option('post_excerpt_maxlength_masonry'))).'</p>';
							}
