...
if ( !function_exists( 'jardiwinery_template_single_matches_theme_setup' ) ) {
	add_action( 'jardiwinery_action_before_init_theme', 'jardiwinery_template_single_matches_theme_setup', 1 );
	function jardiwinery_template_single_matches_theme_setup() {
		jardiwinery_add_template(array(
			'layout' => 'single-matches',
			'mode'   => 'matches',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single matches', 'jardiwinery'),
...
// Template output
if ( !function_exists( 'jardiwinery_template_single_matches_output' ) ) {
	function jardiwinery_template_single_matches_output($post_options, $post_data) {
		...
		$title_tag = jardiwinery_get_custom_option('show_page_title')=='yes' ? 'h3' : 'h1';
		...
		if ($show_title && $post_options['location'] == 'center' && jardiwinery_get_custom_option('show_page_title')=='no') {
			?>
		...
		if (!$post_data['post_protected']) {
			...
			get_template_part(jardiwinery_get_file_slug('templates/trx_matches/_parts/match-info.php'));
		}
		...
		if ($post_data['post_protected']) { 
			...
			echo trim(jardiwinery_gap_wrapper(jardiwinery_reviews_wrapper($post_data['post_content'])));
			wp_link_pages( array( 
		...
		?>
			<div class="match_preview"><a href="<?php echo esc_url($match_preview); ?>"><?php echo esc_html__("Go to the announcement &#8594;", 'jardiwinery'); ?></a></div>
		<?php
		} 
