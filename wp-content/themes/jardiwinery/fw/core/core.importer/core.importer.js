...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		JARDIWINERY_STORAGE['importer_error_messages'] = '';
		JARDIWINERY_STORAGE['importer_ignore_errors'] = true;
		jardiwinery_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function jardiwinery_importer_do_action(steps, idx) {
	"use strict";
	...
	// AJAX query params
	var data = {
		ajax_nonce: JARDIWINERY_STORAGE['ajax_nonce'],
		action: 'jardiwinery_importer_start_import',
	...
	jQuery.post(JARDIWINERY_STORAGE['ajax_url'], data, function(response) {
		...
		try {
			...
			rez = { error: JARDIWINERY_STORAGE['ajax_error']+':<br>'+response };
			console.log(response);
		}
		if (rez.error === '' || JARDIWINERY_STORAGE['importer_ignore_errors']) {
			if (rez.error !== '') 
				JARDIWINERY_STORAGE['importer_error_messages'] += '<p class="error_message">' + rez.error + '</p>';
			...
			// Do next action
			if (idx < steps.length) {
				jardiwinery_importer_do_action(steps, idx);
			} else {
				if (JARDIWINERY_STORAGE['importer_error_messages']) {
					jQuery('#trx_importer_progress').removeClass('notice-info').addClass('notice-error').append('<h4>' + JARDIWINERY_STORAGE['importer_error_msg'] + '</h4>' + JARDIWINERY_STORAGE['importer_error_messages']);
				} else {
