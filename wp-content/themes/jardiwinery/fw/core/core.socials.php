<?php
/**
 * JardiWinery Framework: social networks
 *
 * @package	jardiwinery
...
// Theme init
if (!function_exists('jardiwinery_socials_theme_setup')) {
	add_action( 'jardiwinery_action_before_init_theme', 'jardiwinery_socials_theme_setup' );
	function jardiwinery_socials_theme_setup() {

		if ( !is_admin() ) {
			// Add og:image meta tag for facebook
			add_action( 'wp_head', 'jardiwinery_facebook_og_tags', 5 );
		}
...
jardiwinery_storage_set('share_links', array(
	...
	'postila' =>		'http://postila.ru/publish/?url={link}&agregator=jardiwinery',
...
if (!function_exists('jardiwinery_add_share_link')) {
	function jardiwinery_add_share_link($soc, $url='') {
		if (!is_array($soc)) $soc = array($soc => $url);
		jardiwinery_storage_set('share_links', array_merge( jardiwinery_storage_get('share_links'), $soc ) );
	}
}

// Return (and show) share social links
if (!function_exists('jardiwinery_show_share_links')) {
	function jardiwinery_show_share_links($args) {
		if ( jardiwinery_get_custom_option('show_share')=='hide' ) return '';
		...
		$args = array_merge(array(
			...
			'counters' => jardiwinery_get_custom_option('show_share_counters')=='yes',	// show share counters
			'direction' => jardiwinery_get_custom_option('show_share'),				// share block direction
			'caption' => jardiwinery_get_custom_option('share_caption'),				// share block caption
			'share' => jardiwinery_get_theme_option('share_buttons'),					// list of allowed socials
		...
		if (is_array($args['share']) && count($args['share']) > 0) {
			foreach ($args['share'] as $soc) {
				...
				$url = $soc['url'];
				if (empty($url)) $url = jardiwinery_storage_get_array('share_links', $sn);
				...
				$output .= '<div class="sc_socials_item' . (!empty($args['popup']) ? ' social_item_popup' : '') . '">'
					...
					. '</a>'
					. ($args['type']=='drop' ? '<i>' . trim(jardiwinery_strtoproper($sn)) . '</i>' : '')
					. '</div>';
			}
		}
		$output .= '</div>';
...
if (!function_exists('jardiwinery_prepare_socials')) {
	function jardiwinery_prepare_socials($list, $style='') {
		if (empty($style)) $style = jardiwinery_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons';
		$output = '';
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
		if (is_array($list) && count($list) > 0) {
			foreach ($list as $soc) {
				...
				$icon = $style=='icons' || jardiwinery_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : jardiwinery_get_socials_url(basename($soc['icon']));
				if ($style == 'icons') {
					$parts = explode('-', $soc['icon'], 2);
					$sn = isset($parts[1]) ? $parts[1] : $parts[0];
				} else {
...
if (!function_exists('jardiwinery_get_twitter_data')) {
	function jardiwinery_get_twitter_data($cfg) {
...
if (!function_exists('jardiwinery_prepare_twitter_text')) {
	function jardiwinery_prepare_twitter_text($tweet) {
		$text = $tweet['text'];
...
if (!function_exists('jardiwinery_get_twitter_followers')) {
	function jardiwinery_get_twitter_followers($cfg) {
		$data = jardiwinery_get_twitter_data($cfg); 
...
if (!function_exists('jardiwinery_get_facebook_likes')) {
	function jardiwinery_get_facebook_likes($account) {
		$fb = get_transient("facebooklikes");
		if ($fb !== false) return $fb;
		$fb = '?';
		$url = esc_url(jardiwinery_get_protocol().'http://graph.facebook.com/'.($account));
		$headers = get_headers($url);
		if (jardiwinery_strpos($headers[0], '200')) {
			$json = jardiwinery_fgc($url);
...
if (!function_exists('jardiwinery_facebook_og_tags')) {
	//add_action( 'wp_head', 'jardiwinery_facebook_og_tags', 5 );
	function jardiwinery_facebook_og_tags() {
		global $post;
		if ( !is_singular() || jardiwinery_storage_get('blog_streampage')) return;
...
if (!function_exists('jardiwinery_get_feedburner_counter')) {
	function jardiwinery_get_feedburner_counter($account) {
		$rss = get_transient("feedburnercounter");
		if ($rss !== false) return $rss;
		$rss = '?';
		$url = esc_url(jardiwinery_get_protocol().'://feedburner.google.com/api/awareness/1.0/GetFeedData?uri='.($account));
		$headers = get_headers($url);
		if (jardiwinery_strpos($headers[0], '200')) {
			$xml = jardiwinery_fgc($url);
