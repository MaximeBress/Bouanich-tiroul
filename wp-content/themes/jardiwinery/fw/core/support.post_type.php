...
if (!function_exists('jardiwinery_post_type_theme_setup')) {
	...
	function jardiwinery_post_type_theme_setup() {
		...
		if ($override_key) {
			...
			add_action('add_meta_boxes',		'jardiwinery_post_type_add_meta_box');
			add_action('admin_enqueue_scripts', 'jardiwinery_post_type_admin_scripts');
			// Create meta box
			jardiwinery_storage_set('post_meta_box', array(
				'id' => 'post-meta-box',
...
// Add meta box
if (!function_exists('jardiwinery_post_type_add_meta_box')) {
	//add_action('add_meta_boxes', 'jardiwinery_post_type_add_meta_box');
	function jardiwinery_post_type_add_meta_box() {
		$mb = jardiwinery_storage_get('post_meta_box');
		add_meta_box($mb['id'], $mb['title'], 'jardiwinery_post_type_show_meta_box', $mb['page'], $mb['context'], $mb['priority']);
	}
}

// Callback function to show fields in meta box
if (!function_exists('jardiwinery_post_type_show_meta_box')) {
	function jardiwinery_post_type_show_meta_box() {
		global $post;
		...
		do_action('jardiwinery_action_post_before_show_meta_box', $post_type, $post->ID);
		...
		do_action('jardiwinery_action_post_after_show_meta_box', $post_type, $post->ID);
...
if (!function_exists('jardiwinery_post_type_save_options')) {
	...
	function jardiwinery_post_type_save_options($post_id) {
		...
		// verify nonce
		if ( !wp_verify_nonce( jardiwinery_get_value_gp('meta_box_post_nonce'), admin_url() ) )
			return $post_id;
		...
		$post_options = array_merge(jardiwinery_storage_get('options'), jardiwinery_storage_get_array('post_meta_box', 'fields'));
